...
Also: wheel zoom only over the work area; Save / Save As; bounds select + overlap picker. **Cut list** is net per-member rows (click selects those objects). **Shopping list** is a big-box buy list: net cuts packed onto **6', 8', 12', and 16'** sticks only (no 10' or 20'). Leftover after packing (and 1/8 in kerf) is shown as waste — there is no typed waste %. Posts with no typed height buy 8'. Hangers stay 1:1 grouped. No SKU catalog. No invented IRC spans. Heights and lateral-load settings are not edited in this UI (saved JSON fields are left as-is; Fill may still emit R507.9.2 devices).
