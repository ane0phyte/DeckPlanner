...
The **left column** is a section switcher (same idea as the old right pane): Tools, Inspect, IRC flags, Cut list, Shopping list, and Elevation (layers, directions, decking, project). Only the selected section fills the pane — Tools is not a permanent header. There is no right pane. The canvas uses that width. Unmodified letter hotkeys still work while another section is open; they do not force a tab switch. **Ctrl/Cmd** (or **Alt** if Chrome eats the chord) switches sections: **T** Tools, **I** Inspect, **G** IRC flags, **U** Cut list, **B** Shopping list, **E** Elevation. **Ctrl/Cmd+S** remains save.
...
Also: wheel zoom only over the work area; Save / Save As; bounds select + overlap picker. **Cut list** is net per-member rows (click selects those objects). **Shopping list** is a big-box buy list: net cuts packed onto common stick lengths (2x: 8/10/12/16/20; posts: 8/10/12; decking: 12/16, 20 only if a piece needs it). Leftover after packing (and 1/8 in kerf) is shown as waste — there is no typed waste %. Posts with no typed height buy 8' (shortest common). Hangers stay 1:1 grouped. No SKU catalog. No invented IRC spans. Heights and lateral-load settings are not edited in this UI (saved JSON fields are left as-is; Fill may still emit R507.9.2 devices).
...
Cut list lines stay **net** (no waste). The **shopping list** packs those net cuts onto store stick lengths and shows leftover feet and % per size. Hangers stay 1:1. No typed waste percent. No SKU catalog. Area/linear accessories on the cut list still use **your** coverage (sf/roll, count/box) when present in the file.
